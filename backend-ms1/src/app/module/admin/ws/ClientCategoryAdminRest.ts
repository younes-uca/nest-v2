...
import {ClientCategoryAdminServiceImpl} from "src/app/module/admin/service/impl/ClientCategoryAdminServiceImpl";
...
export class ClientCategoryAdminRest {
    ...
    async findByCriteria(): Promise<ClientCategoryDto[]> {
        const clientCategorys = await this.service.findAll();
        return this.converter.toDtos(clientCategorys);
    }
    ...
    async findPaginatedByCriteria(): Promise<PaginatedList<ClientCategoryDto>> {
        const clientCategorys = await this.service.findAll();
