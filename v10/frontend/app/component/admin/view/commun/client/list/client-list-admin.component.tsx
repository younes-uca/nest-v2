            ...
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            ...
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="2">{t("client.clientCategoryPlaceHolder")}</label>
                                <Dropdown id="2" value={criteria.clientCategory} options={clientCategorys} onChange={(e) => setCriteria({ ...criteria, clientCategory: e.target.value })} optionLabel="reference" filter showClear/>
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="8">{t("client.username")}</label>
                                <InputText id="8" value={criteria.username} onChange={(e) => setCriteria({ ...criteria, username: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="9">{t("client.password")}</label>
                                <InputText id="9" value={criteria.password} onChange={(e) => setCriteria({ ...criteria, password: e.target.value })} />
                            </div>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as ClientDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="credentialsNonExpired" header={t("client.credentialsNonExpired")} body={CustomBooleanCell("credentialsNonExpired")} sortable></Column>
                    
                    
                    <Column field="enabled" header={t("client.enabled")} body={CustomBooleanCell("enabled")} sortable></Column>
                    
                    
                    <Column field="accountNonExpired" header={t("client.accountNonExpired")} body={CustomBooleanCell("accountNonExpired")} sortable></Column>
                    
                    
                    <Column field="accountNonLocked" header={t("client.accountNonLocked")} body={CustomBooleanCell("accountNonLocked")} sortable></Column>
                    
                    
                    <Column field="passwordChanged" header={t("client.passwordChanged")} body={CustomBooleanCell("passwordChanged")} sortable></Column>
                    
                    
                    <Column field="username" header={t("client.username")} sortable></Column>
                    
                    
                    <Column field="password" header={t("client.password")} sortable></Column>
