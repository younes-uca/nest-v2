...
export class ${pojo.name}Dao extends AbstractRepository<${pojo.name}, ${pojo.name}Criteria> {
    ...
    async saveOrUpdate(item: ${pojo.name}): Promise<${pojo.name}> {
        if (item.id) {
...
<#if  pojo.labelOrReference??>
                    .select(['item.id AS id', 'item.${pojo.labelOrReference.name} AS ${pojo.labelOrReference.name}'])
                    .getRawMany()
                    .then((result) => result.map((row) => new ${pojo.name}Dto(row.id, row.${pojo.labelOrReference.name})));
<#else>
                    .select(['item.id AS id'])
                    .getRawMany()
                    .then((result) => result.map((row) => new ${pojo.name}Dto(row.id)));
</#if>
...
<#list pojo.fieldsGeneric as fieldGeneric>
    ...
    async deleteBy${fieldGeneric.name?cap_first}Id(${fieldGeneric.pojo.id.name}: number): Promise<number> {
        const result = await this.repository.delete({${fieldGeneric.name?uncap_first}: {${fieldGeneric.pojo.id.name}}});
        return result.affected;
    }
        </#if>
    </#if>
    ...
    public constructQuery(criteria: ${pojo.name}Criteria): SelectQueryBuilder<${pojo.name}> {
        const query = this.initQuery(this.repository)<#if pojo.fieldsGeneric?size == 0>;</#if>
    <#list pojo.fieldsGeneric as fieldGeneric>
            .leftJoin('item.${fieldGeneric.name}', '${fieldGeneric.name}')
    </#list>
    <#if pojo.fieldsGeneric?size gt 0>
            .select(['item' <#list pojo.fieldsGeneric as fieldGeneric>, '${fieldGeneric.name}'</#list>])
    </#if>
    ...
    <#list pojo.fieldsGeneric as fieldGeneric>
        if (criteria.${fieldGeneric.name}) {
            ...
            this.addConstraint(query, ${fieldGeneric.name}.id, '${fieldGeneric.name}.id = :${fieldGeneric.name}Id', {${fieldGeneric.name}Id: ${fieldGeneric.name}.id,});
            this.addConstraint(query, ${fieldGeneric.name}.${fieldGeneric.typeAsPojo.labelOrReference.name}, '${fieldGeneric.name}.${fieldGeneric.typeAsPojo.labelOrReference.name} = :${fieldGeneric.name}${fieldGeneric.typeAsPojo.labelOrReference.name?cap_first}', {${fieldGeneric.name}${fieldGeneric.typeAsPojo.labelOrReference.name?cap_first}: ${fieldGeneric.name}.${fieldGeneric.typeAsPojo.labelOrReference.name},});
        }
    </#list>
        return query;
    }
