...
import {${pojo.name}${role.name?cap_first}ServiceImpl} from "src/app/module/${role.name?uncap_first}/service/impl/${pojo.name}AdminServiceImpl";
...
export class ${pojo.name}${role.name?cap_first}Rest {
    ...
    async findByCriteria(): Promise<${pojo.name}Dto[]> {
        const ${pojo.name?uncap_first}s = await this.service.findAll();
        return this.converter.toDtos(${pojo.name?uncap_first}s);
    }
    ...
    async findPaginatedByCriteria(): Promise<PaginatedList<${pojo.name}Dto>> {
        const ${pojo.name?uncap_first}s = await this.service.findAll();
...
<#list pojo.fieldsGeneric as fieldGeneric>
    ...
    async deleteMultiple(@Body() dtos: ${pojo.name}Dto[]): Promise<${pojo.name}Dto[]> {
        ...
        const deleted${pojo.name?uncap_first}s = await this.service.deleteMultiple(${pojo.name?uncap_first}s);
