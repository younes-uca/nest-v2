...
<#list pojo.fieldsGeneric as fieldGeneric>
    ...
    async search(searchDto: ${pojo.name}Criteria): Promise<${pojo.name}[]> {
    ...
    <#list pojo.fieldsSimple as fieldSimple>
        ...
        this.addConstraintMinMax(query, searchDto.${fieldSimple.name}From, searchDto.${fieldSimple.name}To, '${fieldSimple.name} >= :${fieldSimple.name}From', '${fieldSimple.name} <= :${fieldSimple.name}To', {${fieldSimple.name}From: searchDto.${fieldSimple.name}From,${fieldSimple.name}To: searchDto.${fieldSimple.name}To,});
            <#elseif fieldSimple.nombre>
        this.addConstraintMinMax(query, searchDto.${fieldSimple.name}Min, searchDto.${fieldSimple.name}Max, '${fieldSimple.name} >= :${fieldSimple.name}Min', '${fieldSimple.name} <= :${fieldSimple.name}Max', {${fieldSimple.name}Min: searchDto.${fieldSimple.name}Min,${fieldSimple.name}Max: searchDto.${fieldSimple.name}Max,});
            <#elseif fieldSimple.bool>
        this.addConstraint(query, searchDto.${fieldSimple.name}, '${fieldSimple.name} = :${fieldSimple.name}', {${fieldSimple.name}: searchDto.${fieldSimple.name}});
    ...
    <#list pojo.fieldsGeneric as fieldGeneric>
        if (searchDto.${fieldGeneric.name}) {
            const ${fieldGeneric.name} = searchDto.${fieldGeneric.name};
            this.addConstraint(query, searchDto.${fieldGeneric.name}.id, '${fieldGeneric.name}.id = :${fieldGeneric.name}Id', {${fieldGeneric.name}Id: ${fieldGeneric.name}.id,});
            this.addConstraint(query, searchDto.${fieldGeneric.name}.${fieldGeneric.typeAsPojo.labelOrReference.name}, '${fieldGeneric.name}.${fieldGeneric.typeAsPojo.labelOrReference.name} = :${fieldGeneric.name}${fieldGeneric.typeAsPojo.labelOrReference.name?cap_first}', {${fieldGeneric.name}${fieldGeneric.typeAsPojo.labelOrReference.name?cap_first}: ${fieldGeneric.name}.${fieldGeneric.typeAsPojo.labelOrReference.name},});
        }
    </#list>
